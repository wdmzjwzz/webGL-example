export class Element<T>{
    ...
    public get root(): Element<T> {
        ...
        while (true) {
            let n: Element<T> | null = root._parent;
            if (n === null) {
                break;
            }
            root = n;
    ...
    public removeChild(old: Element<T>): boolean {
        ...
        let prevChild: Element<T> | null = old._prevSibling;
        let nextChild: Element<T> | null = old._nextSibling;
    ...
    public addChild(child: Element<T>): boolean {
        ...
        let oldParent: Element<T> | null = child._parent;
        if (oldParent !== null) {
    ...
    public moveNext(stopAt: Element<T>): Element<T> | null {
        ...
        while (n !== null && n._nextSibling === null && (stopAt === null || n._parent !== stopAt)) {
            n = n._parent;
        }
        if (n !== null) {
