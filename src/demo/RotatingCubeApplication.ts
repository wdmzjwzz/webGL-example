...
export class RotatingCubeApplication extends CameraApplication {
    ...
    private _renderCube (): void {
        ...
        // 第一个渲染堆栈操作
        {
            ...
            // 使用辅助方法绘制坐标系
            DrawHelper.drawCoordSystem( this.builder, this.camera.viewProjectionMatrix, this._hitAxis, 1 );
    ...
    private _renderTriangle (): void {
        ...
        this.colorProgram.bind();
        {
            ...
            this.builder.begin( this.gl.LINE_LOOP ); // 开始绘制，defatul使用gl.TRIANGLES方式绘制
            this.builder.color( 1, 0, 0 ).vertex( -1, -1, 0 );  // 三角形第一个点的颜色与坐标
            this.builder.color( 1, 0, 0 ).vertex( 1, -1, 0 ); // 三角形第二个点的颜色与坐标
            this.builder.color( 1, 0, 0 ).vertex( 1, 1, 0 ); // 三角形第三个点的颜色与坐标
            this.builder.color( 1, 0, 0 ).vertex( -1, 1, 0 ); // 三角形第三个点的颜色与坐标
